...
public class RabbitConsumer {
     ...
     * 普通消息
     */
    @RabbitListener(queuesToDeclare = @Queue("queue_fanout2"))
    ...
    /**
     * 普通消息
     * 如果将multiple设置为true，则表示确认所有比deliveryTag小或等于deliveryTag的未确认消息。
     * 如果将multiple设置为false，则只确认指定deliveryTag的消息
     */
    @RabbitListener(queuesToDeclare = @Queue(RabbitDefine.DIRECT_QUEUE2),concurrency="3")
    public void consumer_derect_2(String message, Channel channel, @Header(AmqpHeaders.DELIVERY_TAG) long tag) throws IOException {
        log.info("消费者1（直通消息）-队列索引：{},接收到消息：{}", tag, message);
